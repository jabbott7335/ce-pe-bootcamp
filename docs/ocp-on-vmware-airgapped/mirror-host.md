...
1. Add a 500GB disk to the bastion virtual machine.
...
1. List the block devices and identify the new disk.
...
1. Extend volume group `rhel_bastion` with the new block device `/dev/sdb`.
...
1. Extend the size of the root logical volume to 100% of it's unallocated size.
...
1. Extend the size of the root filesystem.
