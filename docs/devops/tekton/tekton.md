...
```
> **Note** - As tasks will use generated name, you won't be able to use the `oc apply -f taskrun-test.yaml` command.
...
> The `-f` or `-a` flags allow to tail the logs from all the containers of the task. For more options run `tkn tr logs --help`
...
!!! question "What is this task doing?"
    This task is not exactly human legible. Let's quickly break it down:

    1. This task is responsible for building container images from a `Dockerfile`. Buildah is an open source, Linux-based tool used to build Open Container Initiative (OCI)-compatible containers.
    2. The task defines a number of parameters to allow users to customise its behaviour, such as setting a custom `Dockerfile` path, or any additional build arguments.
    3. The task runs on a `ubi8` Red Hat image. `Universal Base Image` (UBI) is a Red Hat project to provide secure and lightweight base containers to developers. [Take a minute to learn more about it here](https://www.redhat.com/en/blog/introducing-red-hat-universal-base-image). A number of scripts are loaded into the image to automate working with `Buildah`
    4. The `securityContext` block configures security permissions for the task. By default, tasks will be run as non-root with restricted permissions. OpenShift uses Security Context Constraints to manage pod permissions. [Take a minute to review working with pipelines ](https://docs.openshift.com/pipelines/1.17/secure/configuring-security-context-for-pods.html)
...
Now access the compose the URL of the App using IP and Route:
```bash
