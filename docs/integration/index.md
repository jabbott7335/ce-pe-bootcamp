...
Lastly, this guide assumes you already have an OCP cluster with the right version and capacity (v4.16.x or v4.17.x and at least 80 vCPUs and 320 GB of memory, for best results it is recommended to have 5 worker nodes 32 vCPUs X 128 GB memory each) up and running on IBM Cloud, either in your own account or via [TechZone](https://techzone.ibm.com/environments). You should already have this cluster provisioned and you should be logged in from the CLI before continuing.  If you are note using your IPI on VMware install from the bootcamp you can find further instructions to provision a cluster in TechZone follow this [link](TechZoneGuide.md).
